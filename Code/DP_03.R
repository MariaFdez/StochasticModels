...
ggplot(data=mergeData,aes(x=xcoordinate,y=ycoordinate,col=type))+geom_point()+theme_bw()
n<-nrow(mergeData)
...
nclus<-6
coordinates$klus<-kmeans(coordinates,nclus)$cluster
as.data.frame(coordinates)


coordinates<- rbind(coordinates,c(468885.1451,4200354.6723,8))
coordinates<-rbind(coordinates,c(464323.2597,4204134.6673,9))
ggplot(data=coordinates,aes(x=xcoordinate,y=ycoordinate,col=as.factor(klus)))+geom_point(size=3)+theme_bw()

#meters terms
matrix<-list();cluster<-list(); dist<-list(); tsp<-list(); tour<-list(); cl<-list()
for(i in 1:nclus){
  matrix[[i]]<-coordinates[coordinates$klus==i,]
  cluster[[i]]<-as.matrix(matrix[[i]][,1:2])
  dist[[i]]<-dist(cluster[[i]],diag=T,method="euclidean")
  tsp[[i]]<-TSP(dist[[i]])
  tour[[i]]<-solve_TSP(tsp[[i]])
  cl[[i]]<-cbind(cluster[[i]],order=as.integer(tour[[i]]))
  }
p1<-ggplot(data=as.data.frame(cl[[1]]),aes(x=xcoordinate,y=ycoordinate,label=order))+geom_point()+geom_text(hjust=0, vjust=0)+geom_line()
p2<-ggplot(data=as.data.frame(cl[[2]]),aes(x=xcoordinate,y=ycoordinate,label=order))+geom_point()+geom_text(hjust=0, vjust=0)+geom_line()
p3<-ggplot(data=as.data.frame(cl[[3]]),aes(x=xcoordinate,y=ycoordinate,label=order))+geom_point()+geom_text(hjust=0, vjust=0)+geom_line()
p4<-ggplot(data=as.data.frame(cl[[4]]),aes(x=xcoordinate,y=ycoordinate,label=order))+geom_point()+geom_text(hjust=0, vjust=0)+geom_line()
p5<-ggplot(data=as.data.frame(cl[[5]]),aes(x=xcoordinate,y=ycoordinate,label=order))+geom_point()+geom_text(hjust=0, vjust=0)+geom_line()
p6<-ggplot(data=as.data.frame(cl[[6]]),aes(x=xcoordinate,y=ycoordinate,label=order))+geom_point()+geom_text(hjust=0, vjust=0)+geom_line()
