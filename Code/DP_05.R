...
TSP.cluster<- function(Viol.coordenates, mergeData,i){
  ...
  #cluster<-rbind(cluster,c(-2018264.841,3713016.008,i))
...
### -----------------------------------------------------------------------------
## getting the distance to the nearest violated can
### -----------------------------------------------------------------------------
NonVio.cans<-Cans[-indCans,]
## getting the nearest can
...
time_cost <- 0.15 # euros per hour
dist_cost<-0.75 # cost of each extra meter we do
penalty<-10 # cost if we exceed the max cap of the can

decision<-c()

for(j in 1:nrow(State)){
    cost<-(ind.dist$ExtraDistance[j]*dist_cost)+
        (((NonVio.cans$service[j]/3600)+(ind.dist$ExtraDistance[j]/12000))*time_cost)
  decision[j]<-ifelse((State$Prob[j]*penalty>=cost),1,0)
}
